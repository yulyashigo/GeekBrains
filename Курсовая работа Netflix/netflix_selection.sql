...
/* Задача: Необходимо посчитать в каком месяце было добавлено больше всего фильмов в библиотеку */
...
-- Ответ: в декабре и ноябре

/* Задача: К какому тарифу подключено больше всего пользователей? */

SELECT 
  COUNT(*) as 'Кол-во подключений',
  plan_id,
  (SELECT name FROM tariff_plan tp where tp.plan_id = ut.plan_id) as 'Наименование тарифа'
FROM users_tariff ut 
GROUP BY plan_id;

-- Ответ: К базовому и стандартному

/*Задача: Посчитаем количество фильмов пользователей, добавленых в список просмотров*/

SELECT COUNT(id) AS 'Кол-во фильмов', 
(select id from users u where u.id = vl.user_id) as 'Пользователь'
  FROM view_list vl 
  GROUP BY user_id;
 
 -- Ответ: Каждый пользователь сейчас смотрит по 2 фильма
 
/*Задача: Определить предпочтения в фильмах пользователя с id=2*/ 
 
SELECT fs.id_movie, 
ml.name AS 'Наименование фильма', 
fg.name AS 'Наименование жанра',
fs.score AS 'Оценка', 
fs.user_id, 
u.firstname AS 'Имя', 
u.lastname AS 'Фамилия', 
fs.created_at AS 'Дата оценки'
  FROM film_score fs 
    JOIN users u ON fs.user_id = u.id 
    JOIN movie_library ml ON fs.id_movie = ml.id 
    JOIN film_genre fg ON ml.id_genre = fg.id 
  WHERE fs.user_id = 2;
 
 -- Ответ: Пользователю id=2 не нравится фильм "asperiores" жанра Голливуд, но нравится аниме "illo"
 

/* 3. Задача: Вывести личный список фильмов пользователя (id=1)  с наименованием жанров */

SELECT 
ml.name AS 'Наименование фильма',
fg.name AS 'Жанр', 
ml.`type` AS 'Тип', 
ml.year_issue 'Год выхода', 
ul.user_id, 
u.firstname AS 'Имя', 
u.lastname AS 'Фамилия'
FROM film_genre fg 
	JOIN movie_library ml ON  fg.id = ml.id_genre 
	JOIN users_list ul ON ml.id = ul.id_movie 
	JOIN users u ON ul.user_id = u.id 
WHERE ul.user_id = 1;

-- Ответ: в список фильмов пользователя id=1 вошли фильмы: "quidem" жанра "Детские и семейные" и сериал "excepturi" жанра ужастики
