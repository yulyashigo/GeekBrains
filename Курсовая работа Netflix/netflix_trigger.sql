...
begin
    IF NEW.birthday >= CURRENT_DATE() THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Будьте внимательны! Дата рождения должна быть прошедшего времени';
    END IF;
END//

DELIMITER ;

/*При удалении фильма из библиотеки он удаляется из таблицы оценки фильмов*/

DELIMITER //
CREATE TRIGGER delete_films_film_score BEFORE DELETE ON movie_library
FOR EACH ROW BEGIN
  DELETE FROM film_score fs WHERE ml.id =OLD.id;
END//
DELIMITER ;

/*При удалении фильма из библиотеки он удаляется из списка просмотров пользователя*/

DELIMITER //
CREATE TRIGGER delete_films_view_list BEFORE DELETE ON movie_library
FOR EACH ROW BEGIN
  DELETE FROM view_list vl WHERE ml.id =OLD.id;
END//
DELIMITER ;

/*При удалении фильма из библиотеки он удаляется из списка пользователя*/

DELIMITER //
CREATE TRIGGER delete_films_users_list BEFORE DELETE ON movie_library
FOR EACH ROW BEGIN
  DELETE FROM users_list ul WHERE ml.id =OLD.id;
END//
DELIMITER ;

-- ХРАНИМЫЕ ПРОЦЕДУРЫ

/*Добавим процедуру добавления нового пользователя*/

DROP PROCEDURE IF EXISTS new_film;

DELIMITER //
CREATE PROCEDURE ins_users(f VARCHAR(50), l VARCHAR(50), e VARCHAR(120), p BIGINT(20))
     BEGIN
        INSERT INTO users  (firstname, lastname, email, phone) value (f, l, e, p);
     END
DELIMITER ;  
